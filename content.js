...
function onKeyDown(e) {
  ...
  let char = e.key;
  if (char.length > 1) return;  //system key like "Shift" or "Backspace"
...
function processNext() {
  ...
  if (queue.length >= 3) {
    const input = "\n" + queue.join("");
    let i = 4;  //BIP319 has min 3 chars (+ 1 leading CRLF char)
    const start = bip319.indexOf(input.substring(0, i));
    if (start >= 0) {
      //truncate the word list to make searching faster
      const section = bip319.substring(
        start,
      ...
      if (i <= len) {
        ...
        if (section.includes(input.substring(0, i) + "\n")) {
          ...
          if (matches.length > 4) {
            if (!warned) {
              warned = true;
              window.open(chrome.runtime.getURL("careful.html"), "_blank");
            }
          }
